...
- quest-approach menus now let the player read the scene with `Insight` before committing; that read gives guidance about likely approaches and class strengths, but the final quest result still comes from a chosen method such as `Intimidation`, `Persuasion`, `Survival`, `Investigation`, or `Performance`
