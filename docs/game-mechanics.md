...
- camps persist by monster-zone position instead of being torn down when the hero returns to town; on later return, days away, local danger, and weather can degrade the old camp by one or more levels, with hidden and fortified camps resisting that pressure better
...
- status details split core hero state and weapon math into separate rows so the player can scan HP, AC, inventory, hit bonus, and damage without one dense terminal line wrapping awkwardly
...
- first-time town flow now adds early orientation: Work & Trouble is called out as the main investigation hub, Guard Station and Quest Giver are framed as the required opening story pair, and Quest Board/day jobs are framed as optional coin and practice
- the second first-time-flow pass makes the same naming visible in the actual town menu, marks Work & Trouble as the recommended first story entry, and explains the daily story lock by naming which opening lead becomes useful after the next inn rest
- town relationship hints surface active inn, street, and social payoffs beside the main progression reminder, such as Bard private-salon openings, Belor market permits, Fighter tourney-patron attention, armorer favors, Barbarian mercenary/healing leads, active Silver Kettle payout insight, and Bent Nail under-table leads
- the same current relationship hint is also carried into status and quest-log views, so opened social payoffs stay visible while the player checks hero state or accepted work
...
- Silver Kettle economic insight is a one-time payout bonus on the next paid quest completion; once spent, the town relationship hint stops advertising that payout
...
- `Docks Tier 5`: `The Civic Vault` opens as a dungeon-style climax under the Civic Keep, using room navigation, combat encounters, searchable clues, loot, locked cache rewards, short-rest rooms, a mid-dungeon witness-box loop, and a late oath-gallery side room
