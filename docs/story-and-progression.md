...
- keep the current relationship hint visible from status and quest-log views, not only the town landing menu
...
The first-time town flow now makes the opening structure more explicit in play: the town hub points new players toward `Work & Trouble`, the work menu names `Guard Station` and `Quest Giver` as the required opening story pair, and source-level hints explain that `Quest Board` and day jobs are mostly coin/practice rather than the first story gate. The second pass also keeps that naming consistent in the actual town menu and, when a story quest has already moved today, tells the player to rest before naming the specific remaining opening lead or deeper Work & Trouble follow-up.
...
- Auntie Brindle is intended to become the monster-part magic crafter after the monster zone produces enough strange components; the town `Arcane Curios` shop now previews expensive finished magic, including broader stat curios, while Auntie should eventually make stranger, component-driven versions from delivered oddities
