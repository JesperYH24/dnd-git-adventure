...
function Test-OlderSaveDataGetsNewDefaultsOnLoad {
    ...
    try {
        ...
        Assert-True -Condition $loadedGame.Town.ContainsKey("Mounts") -Message "Older saves should gain mount ownership defaults."
        Assert-True -Condition $loadedGame.Town["Mounts"].ContainsKey("Owned") -Message "Older saves should gain nested mount ownership defaults."
        Assert-Equal -Actual $loadedGame.Town["Mounts"]["MonsterOddityCapacity"] -Expected 0 -Message "Older saves should start with no monster oddity hauling capacity."
        Assert-True -Condition $loadedGame.Town.ContainsKey("MonsterZone") -Message "Older saves should gain monster zone defaults."
